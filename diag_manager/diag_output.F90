...
!> Set up the time.  Static field and default time is 0
     ...
     if (size(buffer,2) .eq. 1 .and. size(buffer,3) .eq. 1) then
        !> If the variable is 1D, switch the 2nd and 4th index, so local_buffer
        ! has dimensions (nx, n_diurnal_samples, 1, 1).
        allocate(local_buffer(size(buffer,1),size(buffer,4),size(buffer,2),size(buffer,3)))
        local_buffer(:,:,1,1) = buffer(:,1,1,:)
     elseif (size(buffer,3) .eq. 1) then
        !> If the variable is 2D, switch the 3rd and 4th index, so local_buffer has
        ! dimensions (nx, ny, n_diurnal_samples, 1).
