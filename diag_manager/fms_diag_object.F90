...
subroutine fms_diag_do_io(this, end_time)
  ...
  do i = 1, size(this%FMS_diag_files)
    ...
    buff_loop: do ibuff=1, SIZE(buff_ids)
      ...
      call get_model_time(model_time, diag_buff%get_buffer_time())
    ...
    if (present (end_time)) then
      ! ovewrite the model time with the end_time if it is the end of the run
      force_write = .true.
      model_time => end_time
    endif

    if (.not. associated(model_time)) then
      ! If the model time was not set, it is not time to write
...
subroutine get_model_time(current_model_time, buffer_time)
  ...
  elseif (buffer_time > current_model_time) then
    current_model_time => buffer_time
  endif
