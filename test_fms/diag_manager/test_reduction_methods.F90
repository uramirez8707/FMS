...
program test_reduction_methods
  ...
  !< Register the fields
  id_var0 = register_diag_field  ('ocn_mod', 'var0', Time, 'Var0d', 'mullions')
  id_var1 = register_diag_field  ('ocn_mod', 'var1', (/id_x/), Time, 'Var1d', 'mullions')
  id_var2 = register_diag_field  ('ocn_mod', 'var2', (/id_x, id_y/), Time, 'Var2d', 'mullions')
  id_var3 = register_diag_field  ('ocn_mod', 'var3', (/id_x, id_y, id_z/), Time, 'Var3d', 'mullions')
  id_var4 = register_diag_field  ('ocn_mod', 'var4', (/id_x, id_y, id_z, id_w/), Time, 'Var4d', 'mullions')
...
!$OMP parallel do default(shared) private(iblock, isw, iew, jsw, jew, is1, ie1, js1, je1)
      ...
      !--- indices for 1-based arrays ---
        ...
        case (real_mask)
          used=send_data(id_var1, cdata(isw:iew, 1, 1, 1), time, is_in=is1, ie_in=ie1, &
            rmask=crmask(isw:iew, 1, 1, 1))
          used=send_data(id_var2, cdata(isw:iew, jsw:jew, 1, 1), time, is_in=is1, js_in=js1, &
            rmask=crmask(isw:iew, jsw:jew, 1, 1))
          used=send_data(id_var3, cdata(isw:iew, jsw:jew, :, 1), time, is_in=is1, js_in=js1, &
            rmask=crmask(isw:iew, jsw:jew, :, 1))
        case (logical_mask)
          used=send_data(id_var1, cdata(isw:iew, 1, 1, 1), time, is_in=is1, ie_in=ie1, &
            mask=clmask(isw:iew, 1, 1, 1))
          used=send_data(id_var2, cdata(isw:iew, jsw:jew, 1, 1), time, is_in=is1, js_in=js1, &
            mask=clmask(isw:iew, jsw:jew, 1, 1))
          used=send_data(id_var3, cdata(isw:iew, jsw:jew, :, 1), time, is_in=is1, js_in=js1, &
            mask=clmask(isw:iew, jsw:jew, :, 1))
        end select
      enddo
    end select
