  ...
  subroutine check_data_1d(buffer, time_level)
    ...
    do ii = 1, size(buffer, 1)
      buffer_exp = real((real(ii, kind=r8_kind)* 1000_r8_kind+10_r8_kind+1_r8_kind)*6_r8_kind + &
                   real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
      if (use_mask .and. ii .eq. 1) buffer_exp = 0_r4_kind
    ...
    do ii = 1, size(buffer, 1)
      do j = 1, size(buffer, 2)
        buffer_exp = real((real(ii, kind=r8_kind)* 1000_r8_kind+ &
                     ...
                     real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
        if (use_mask .and. ii .eq. 1 .and. j .eq. 1) buffer_exp = 0_r4_kind
      ...
      do j = 1, size(buffer, 2)
        do k = 1, size(buffer, 3)
          buffer_exp = real((real(ii+nx_oset, kind=r8_kind)* 1000_r8_kind + &
                       ...
                       real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
          if (use_mask .and. ii .eq. 1 .and. j .eq. 1 .and. k .eq. 1 .and. .not. is_regional) buffer_exp = 0_r4_kind
